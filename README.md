...
Formulated as a binary classification task with $\mathtt{is\_at\_risk} \in \{0,1\}$, the model achieves evidence-based insights intended
...
Below we report the Pearson correlations between each numerical feature and the target $\mathtt{is\_at\_risk}$.
